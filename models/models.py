...
class ModelsFactory:
    ...
    def get_by_name(model_name, *args, **kwargs):
        ...
        if model_name == 'LRPPN':
            from .LRPPN import LRPPNet
            model = LRPPNet(*args, **kwargs)
        else:
...
class BaseModel(object):
    ...
    def _load_network(self, network, network_label, epoch_label):
        ...
        load_path = os.path.join(self._save_dir, load_filename)
        assert os.path.exists(
            load_path), '%d Weights file not found. Have you trained a model!? We are not providing one' % load_path
    ...
    def _get_scheduler(self, optimizer, opt):
        if opt.lr_policy == 'lambda':
            def lambda_rule(epoch):
                ...
                return lr_l
            scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda epoch: opt.lr_change**epoch, last_epoch=opt.load_epoch or -1)
        elif opt.lr_policy == 'step':
            scheduler = lr_scheduler.StepLR(optimizer, step_size=opt.lr_decay_iters, gamma=opt.lr_gamma, last_epoch=opt.load_epoch or -1)
