...
class discriminator_e(NetworkBase):
    """behaviour Discriminator"""
    def __init__(self, opt, conv_dim=64, repeat_num=2):
        super(discriminator_e, self).__init__(opt)
        self._name = 'discriminator_e'
        ...
        for i in range(repeat_num) :
            ...
            conv_layers.append(self._get_output_function(True))
            cur_dim = cur_dim // 4
            cur_size = cur_size // 2
        ...
        linear_layers.append(nn.Linear(512, opt.expression_type))
    ...
    def forward(self, x):
        ...
        behaviour_logit = self.linear(feature_maps.view(feature_maps.size(0), -1))
