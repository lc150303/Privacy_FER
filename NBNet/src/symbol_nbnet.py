...
def get_symbol(
    ...
    data = mx.symbol.Variable(name='data')      # 输入是 （batch_size, 128)
    # conv = mx.symbol.FullyConnected(data, num_hidden=25600, name='fl0')
    # conv = mx.symbol.Activation(data=data, act_type='relu', name='relu_fl0')
    conv = mx.symbol.FullyConnected(data=data, num_hidden=128, name='fc1')
    conv = mx.symbol.Activation(data=conv, act_type='relu', name='relu_fc1')
    conv = mx.symbol.Reshape(conv,shape=(0,0,1,1))  # 从 (batch_size, 128) 变成 (batch_size, 128, 1, 1)
    conv = mx.symbol.Deconvolution(name="conv0",data=conv,num_filter=n_channels,kernel=(5, 5),no_bias=True)
