...
from LRPPN.LRPPN_model import LRPPN
...
def LRPPN_feature_extractor(args, devs:list, model_path:str, img_size, is_HR):
    assert model_path, 'model_path [%s] invalid' % model_path
    parallel_models = []
    for i_dev,dev in enumerate(devs):
        parallel_models.append(LRPPN(args, is_HR, model_path, dev, img_size))

    def single_feature_extract(i,image,out_list):
        out_list[i] = parallel_models[i].get_feature(image).numpy()

    if len(devs) <= 1:
        def single_extract(images):
            out_list = [0]
...
def feature_extractor(devs,model_path=facenet_path):
    model_path = model_path or facenet_path
    iph = []; ebds = []; ptph = []; ss = []
    for i_dev,dev in enumerate(devs):
    ...
    if len(devs) <= 1:
        def f_extract(images):
            images = np.concatenate([np.expand_dims(prewhiten(x), axis=0) for x in images])
            out_list = [0]
            single_feature_extract(0,images,out_list)
            return out_list[0]
        return f_extract
...
class RandFaceIter(mx.io.DataIter):
    def __init__(self, batch_size, img_gen, fea_ext, ndim=100, max_count=5e3, feature_size=128):
        self.batch_size = batch_size
        self.ndim = ndim
        self.img_gen = img_gen
        self.fea_ext = fea_ext
        self.counter = 0
        self.maxcount = np.int_(max_count) # for calling the epoch_end
    ...
    def iter_next(self):
        ...
        data = np.clip((self.cur_img.transpose((0,2,3,1))+1.0)/2.0, 0, 1)   # b*C*H*W -> b*H*W*C
        self.cur_reps = self.fea_ext(data)
        return True
